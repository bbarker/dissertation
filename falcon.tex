...
\REQUIRE $X$ := A Boolean rule without negation consisting of\\
...
\WHILE{$rule \neq o_1 \land \ldots \land o_p$ where each $o_i$ is a disjunction
  ...
  \STATE Create sets from conjunctions, i.e.:\\
    ...
    \hspace{4.8 mm} $\ANDw_{g_c \in G_C} g_c \land (\mathcal{M}(G_1) \lor 
                     \mathcal{M}(G_2)$ where\\
    \hspace{4.8 mm} $\mathcal{M}\left(S\right) := \argmin_{g \in S}{\E{g}}$\\
