...
~
\REQUIRE $G = \left\{g_i~\mid~i \in{1, \ldots, m}\right\}$ are genes. 
\REQUIRE $r$ := A Boolean rule without negation consisting of\\
...
\WHILE{$rule \neq o_1 \land \ldots \land o_p$ where each $o_i$ is a disjunction
  ...
  \STATE Create sets from conjunctions, i.e.:\\
    ...
    \hspace{4.8 mm} Create $G_C = G_1 \cap G_2$, $G_{1\setminus C} = G_1 \setminus G_C$ and\\ 
    \hspace{4.8 mm} $G_{2\setminus C} = G_2 \setminus G_C$.\\
    \IF {$G_{1\setminus C} = \varnothing || G_{2\setminus C} = \varnothing$}
      \STATE Substitute original disjunction with $G_C$.\\ 
    \ELSE 
      \STATE Substitute original disjunction with:\\
      \hspace{4.8 mm} $\ANDw_{g_c \in G_C} g_c \land (\mathcal{M}(G_{1 \setminus C}) \lor 
                       \mathcal{M}(G_{2\setminus C})$ where\\
...
\ENDWHILE
\ENSURE $o_{\min}$ where $o_{\min}$ has the form: $\ORw_{g_i \in G} g_i$
\end{algorithmic} 
\end{Algorithm}
\end{AlgFloat}
...
in the conditional by removing 
A slightly more detailed proof is given in the
