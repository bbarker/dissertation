...
imply $e_i = e_j$.  The standard deviation of enzyme intensity,
$\sigma_i$, is an optional weighting of uncertainty in biological or
...
\ifthenelse{\boolean{thesisStyle}}{\singlespacing}{}
~\\
$u_{min} = min_j\ V_{j,max} $ s.t. $V_{j,max} > 0$\\
$V_{lb}^{\Sigma} = u_{min} \sum\nolimits_{j=1}^N 
  isIrreversible(v_j) \land exists(e_j)$\\
\FORALL {j}
...
\WHILE{$rxns_{irrev} > rxns_{irrev,prior}$}
  ...
  \INDSTATE $\textnormal{minimize}\ \sum\limits_{i=1} \frac{d_i}{n
    \sigma_i}$ \\
  \INDSTATE s.t. \\
