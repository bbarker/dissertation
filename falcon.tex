...
\[ \textnormal{minimize}\ \sum^N_{i=1}(v_i-a_i)^2 \] subject to the
...
% can we use this without the bioinfo document clasm:?
%\begin{methods}
\section{Methods}
...
%% } {
  ...
  %\processtable{testing\label{ECAssume}} {
  %\begin{center}
  \begin{tabular}{| p{15cm} |}
  \hline
  \textbf{Table ~\ref{ECAssume} Assumptions in GPR-based Enzyme Complex Formation} \\
%%}
\hline
\ifthenelse{\boolean{thesisStyle}}{\begin{enumerate}} {}
\ifthenelse{\boolean{thesisStyle}}{\item} {} \begin{assume} \label{asm:expcorr}
Expression values are highly correlated with the copy numbers of their
corresponding peptide isoforms.
\end{assume}
\ifthenelse{\boolean{thesisStyle}}{\item} {} \begin{assume} \label{asm:isozyme} 
Protein isoforms contributing to isozymes are considered part of the
same enzyme complex.
\end{assume}
\ifthenelse{\boolean{thesisStyle}}{\item} {} \begin{assume} \label{asm:hierarchy}
...
\end{assume}
\ifthenelse{\boolean{thesisStyle}}{\item} {} \begin{assume} \label{asm:nostoich} 
Assume one copy of peptide per complex; exact isoform stoichiometry
is not considered.
\end{assume}
\ifthenelse{\boolean{thesisStyle}}{\item} {} \begin{assume} \label{asm:sharing} 
...
\end{assume}
\ifthenelse{\boolean{thesisStyle}}{\item} {} \begin{assume} \label{asm:active_site}
There is only one active site per enzyme complex.
\end{assume}
\ifthenelse{\boolean{thesisStyle}}{\item} {} \begin{assume} \label{asm:holo} 
...
\begin{table}[!t]
%So we know processtable is required to make the
%table numbers
...
assumption~\ref{asm:nostoich}. It would be surprising if such a
