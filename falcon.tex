...
complexes (\textbf{Figure~\ref{ECCN_flowchart}}). We develop an algorithm for
...
experimentally-based constraints on flux when available (\textbf{Figure
FluxBars}).
...
heavily constrained (\textbf{Figure FluxBars; Figure EnzAbundEval}).
...
than any similarly performing method (\textbf{Table FalcPerf; Figure FluxBars}).
...
highly constrained model (\textbf{Figure YpermCorr}). 
...
FBA; \textbf{Table FalcPerf}) are used. Additionally, it is possible to obtain
good predictions even without a heavily constrained model (\textbf{Table
FalcPerf}).
