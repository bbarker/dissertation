...
(Table~\ref{Table XXX}. The first comparison (using unpermuted data)
resulted in 14 reactions with different expression levels, and after
11 permutations, 39 reactions were observed to have differing
expression. The number of differences did not change between 11 and
1,000 permutations of the expression data. In Human Recon 2, where GPR
...
quantiatively, it may be worth standardizing the form GPR rules
...
programming approach, making it easy for the user to experiment
with. A simple exmample is that the parser is a separate function from
...
Algorithm~\ref{alg:FALCON} recovers the exact glucose uptake (which is
fixed in the prior method) for heavily constrained models, and usually
recovers a similar flux with a small amount of variation in the
minimally constrained models (Figure~\ref{XXX}). We note that this does
not downplay the value of obtaining experimentally-based constraints
on flux when available.
...
issue, but the order in which reactions assigned to be irrersible can
lead to alterantive solutions. However, we
found that the variation due to this stochasticity is relatively minor,
particularly in cases where the model is more heavily constrained.
(Table~\ref{Table XXX}; \suppOrApp Table~\ref{STable XXX};
Figure~\ref{XXX}).
...
perturbation (Figure ExpSens). It is important to note that mere
...
model. Still, the constraints for the human model appear to be not
enough on their own, and more sophisticated means of constraining the
human model, such as enzyme crowding constraints \citep{Shlomi2011},
or using FALCON in conjunction with tissue specific modeling tools,
may prove necessary.
