...
(\textbf{Figure~\ref{ECCN_flowchart}}). We develop an algorithm for
...
(\suppOrApp Figure~\ref{fig:FluxBars}).
...
heavily constrained (\suppOrApp Figure~\ref{fig:FluxBars}; Figure EnzAbundEval).
...
(a) \hspace{1cm} Max. $\mu$ & Model & Experimental & Standard FBA & Fitted FBA & GIMME & iMAT & Lee et al & FALCON\\
...
\end{tabular}
\begin{tabular}{rrrrrrrrr}
(b) \hspace{1cm} Max. $\mu$ & Model & Experimental & Standard FBA & Fitted FBA & GIMME & iMAT & Lee et al & FALCON\\
...
yeast exometabolic fluxes (a) and associated timing analysis (b). For
...
model to expression perturbation (\textbf{Figure ExpSens}). It is important to
...
important (\textbf{Figure ExpSens}).
...
\begin{tabular}{c}
  ...
  \vspace{3mm} \caption*{\hspace*{4cm}\textbf{(a)}} \label{fig:ExpSens:A}
  \end{subfigure}
\\
  \begin{subfigure}[b]{0.5\textwidth}
  \vspace{-5mm} 
  \includegraphics[width=\textwidth]{noise_y7HCflux}
  \vspace{3mm} \caption*{\hspace*{4cm}\textbf{(b)}} \label{fig:ExpSens:B}
  \end{subfigure} 
\\
  ...
  \vspace{3mm} \caption*{\hspace*{4cm}\textbf{(c)}} \label{fig:ExpSens:C}
  \end{subfigure} 
\\
\end{tabular}
\caption{
...
constraint set supplies some benefit (\textbf{Figure ExpSens E-I}), but even
...
(\textbf{Figure FluxExpCmp}). Recently it has been shown that many fluxes are not
