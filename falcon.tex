...
coefficients). The objective may be equivalently expressed in the canonical
...
~
\REQUIRE $\left\{g_i~\mid~i \in{1, \ldots, m}\right\}$ are genes. 
\REQUIRE $\left\{x_i~\mid~i \in{1, \ldots, n}\right\}$ are 
 expressions in Boolean logic.
\WHILE{$rule \neq o_1 \land \ldots \land o_p)$ where each $o_i$ is a disjunction
  of genes} 
  \IF {Encounter a disjunction of conjunctions of genes}
  \STATE  $\mathbf{1.1}$: Create sets from conjunctions, i.e.:\\
    \hspace{4.8 mm} Create $G_1$ and $G_2$ with $g_{1,i} \in G_1$ and $g_{2,i} \in G_2$ where\\ 
    \hspace{4.8 mm} $(g_{1,1} \land \ldots \land g_{1,r}) \lor 
                     (g_{2,1} \land \ldots \land g_{2,s})$. \\
    \hspace{4.8 mm} Create $G_C = G_1 \cap G_2$, $G_{1-C} = G_1 - G_C$ and\\ 
    \hspace{4.8 mm} $G_{2-C} = G_2 - G_C$.\\
    \hspace{4.8 mm} Substitute original disjunction with:\\
    \hspace{4.8 mm} $\ANDw_{g_c \in G_C} g_c \land (\mathcal{M}(G_1) \lor \mathcal{M}(G_2)$ where\\
    \hspace{4.8 mm} $\mathcal{M}\left(g\right) := \argmin E\left(g\right)$\\
  ...
  \STATE $\mathbf{2}$: Distribute $\lor$ over $\land$, e.g.: $(x_1 \land x_2) 
          \lor (x_3 \land x_4)$ \\ 
...
highly constrained model (Figure YpermCorr). To further understand the
...
experimental fluxes when other types of methods (e.g.\ FBA; Table FalcPerf) 
are used. However, as shown in the previous figures, it is
