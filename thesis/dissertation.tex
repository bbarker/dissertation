...
constraints from the $S$ matrix create a cone of feasible solutions
(blue). Linear programming algorithms find an optimal solution on a
vertex (illustrated with orange circle). \textbf{(b, c:)} Depiction of
a simple metabolic network with compartmentalization and its
associated stoichiometric matrix. The three compartments denoted with
subscripts $b$, $e$ and $c$ represent the boundary, extracellular
...
maintenance constraint is not required (see Table~\ref{tab:CBMmethods} and \Fig~2 for
...
difficult to predict, due to error propogation in growth rates.
\\
\end{tabular}}
\end{center}
