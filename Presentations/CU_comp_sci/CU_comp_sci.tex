...
- one of the computational groups in Biology at Cornell.  With my
prior training in computer science theory, software implementation,
server administration, and collaborating with researchers in diverse
fields like biology, medicine, physics, statistics, mathematics, and
computer science, I hope to continue aiding in the exciting and
important research that happens at Cornell.
\par

Our goal in this research and much of my thesis is
to build tools enabling the computation of a fundamental metabolic,
quantity, called flux, in models that may help us to understand disease as
well as various microbial interactions, which are somewhat easier
...
a database, training its users, and extending its features. The most
...
non-embarassingly parallel execution (using OpenMP).  Comparative
genomics research was also part of the position, and that relied
heavily on the UNIX style of data processing. Finally, I initiated
collaboration with a statistician on tree of life problems during 
this time.
\par
...
\note{\scriptsize
One way to describe metabolism is as the set of reactions and the rate
of the reactions made possible by enzymatic catalysts.  The rate of a
reaction is called flux, and if the reactions are the roads, then the
flux is the throughput of traffic on the road. Enzymes, which are
protein catalysts, can be thought of as the width of the road.
...
thousands of these for some classes. Also, reactions thought to be
unimportant often are not drawn.
\par

\textbf{Due to all this complexity and the inability to measure
directly most fluxes, as well as the need to quantitatively understand
...
metabolite, which is here constrained to be zero and is called
...
extendable, and in this work, we extend it by using two
...
flux fitting. Flux fitting can be thought of in terms of our road
analogy as a traffic jam or construction work causing a detour. We
want the detour to be as short as possible while getting us to our
original destination.
\par

In actuality, this is very similar to the first application, which was
to mutate a model by knocking out certain reactions to see what the
...
into proteins. Since reading a template and writing a protein takes
time and energy, the the number of these RNA template copies roughly
corresponds to the amount of protein created. Quantities of both RNAs
...
order to be functional. Here is visualization of a famous protein
...
\note{\scriptsize
This relationship between necessity or redundancy of a gene in a
...
enzyme complex quantification 'min disjunction'.
\par}
...
\note{
Now that know the kind of rule we need, we need to write software that
...
using the original regular expression framework (once we had fixed it
to do so), whereas using our framework takes far less than a second.
}% end of \note
...
variant, and the last block, which is the one most users will
...
abstract syntax tree. Since users may be interested in various levels
of experimentations (including ourselves), the first function, which
...
the outer list is taken to be joined by an AND, and the inner list
joined by ORs. This has the additional benefit of preventing
operations that should only be performed on CNFized rules from being
performed on more general rule types. This function, which returns a
...
normal form. Finally we have a function taht takes a map of gene names
...
function shown above in our StandardML language, with the additional
functionality of identifying which reactions belong to a specific
enzyme complex group, which we will discuss soon. That return values
of computeMinDisj are iputs to falcon, which will estimate a flux
vector.
\par
...
the first part of the framework has when substituted with the
previously used direct substitution method for enzyme complex
quantification.  The x axis of both figures shows flux values when
calculated from direct substituion, and the y-axis from our minimum
disjunction method.  We can see that there is relatively little effect
for yeast, which has simple rules compare dto the human model, where
...
of the problem into a linear fraction program. After this step, we
...
and backward flux, $v_f$ and $v_b$, that can only be non-zero.
This can be thought of in terms of our road analogy again: we aren't
...
want to build our gas stations on.  I promise, that will be the last
time I use the road analogy.  If there is a preference in direction,
we assing the other direction to be zero.
\par

If the number of irreversible reactions increased, then we repeat the
process.  But if nothing changed since the last time, nothing will
...
due to there being fewer constraints overall (there's one contraint
...
\note{
When it comes to predicting performance, we used a benchmark that
...
conditions largely effectect the methods in the panel. While all of
...
predecessor. As can be seen in yellow, the Lee method performs quite
well when the model is highly constrained, which are the default
constaints on directionality that ship with the yeast model. When
these are largely removed, it loses much of its performance, whereas
our method retains good corrleation.
\par}
...
\note{
We also wanted to improve the efficienncy of the method, primarily to
...
it takes about a minute to estimate a flux with FALCON and over an
hour with the Lee method. All of thse benchmarks were done on a single
...
demonstrate this, we hope that we can demonstrate that garbage in gives
us garbage out.
\par
...
experimental data is still good. This gives us a distribution of
correlations, and actually two distributions are plotted here for
...
dynamic genetic interaction here.
}
