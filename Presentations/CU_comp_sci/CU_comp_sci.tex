...
I'd always been interested in biology in addition to my computing
interests, and began working as a cluster administrator, which soon
...
school focused on optimization and dynamical systems, the former of
which was helpful in designing a highly efficient and more accurate
algorithm which I'll mention today, and the latter of which was
helpful to keep myself grounded in reality.
\par
}
...
assumptions about it, such as the steady state condition shown here,
which asserts that concentrations do not change over time. We can
use an optimization objective, such as the maximization of growth
rate for many microbes.
\par
...
\includegraphics*[width=\textwidth, viewport=411 793 643 938, clip=true]
{SmatExample}
\\
\includegraphics*[width=\textwidth, viewport=0 712 631 1130, clip=true]
{SmatExample}
\end{column}
\end{columns}
}
...
would be equal. Now, we no longer need to force flux through any
particular reaction either.

Batch evaulation is another majore benefit as it increases the perfomrance
enough to allow many perturbations to be peformed with ease on microbial
models, and it has allowed simulating human cells to become much more
practical (we're down from about an hour to three and a half minutes).

While both our method and a prior method are somewhat nondeterministic,
we added capabilities to both methods to take this nondeterminism 
into account, which are represented by the error bars that you
have seen in the scatter plots.
...
\note{To introduce this slide, may be good to include one of the correlation
figures between expression and proteomics for instance}.
...
\frame{\frametitle{Sensitivity to expression noise: HC} 
\begin{figure}
\centering
