...
in converting to CNF that we need is the distrubitive expansion of 
...
is simplified to $\ORw G_C$. Otherwise, a direct route is taken to compute
distributive law for the literals: 
$\ANDw_{g_c \in G_C} g_c \land (\ORw_{g_i \in G_1\setminus C) \lor \mathcal{M}(G_2\setminus C)$

$\ANDw_{g_c \in G_C} g_c \land (\mathcal{M}(G_{1 \setminus C}) \lor 
                     \mathcal{M}(G_{2\setminus C})$

Let us first consider when expressions $x_1, ..., x_4$
are all distinct literals, and that
$\E{x_i}$ denotes the expression measurement for gene $x_i$. Assume
...
\subsection{Constraint sets for Human Recon 2}
\label{ssec:humancons}
\hl{it is mostly empty; reference scripts.}
The most highly constrained human model in this study is not nearly
as constrained as the most highly constrained yeast model.
