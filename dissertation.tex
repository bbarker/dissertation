...
The modeling of enzyme complex copy number can be thought of as nested
(but terminating) sets: each enzyme complex consists of
several functional groups of protein subunits. These functional groups
are required for the enzyme complex to function (AND
relationships). However, there may be alternative versions of each
functional set (given by OR relationships). Eventually, the
recursion terminates with a base-level functional set (i.e. a set of
protein isoforms--usually only one--fulfilling the same function). 
In the case of ATP Synthase, one of its functional sets is represented
by the F1 subcomplex. The F1 subcomplex itself can be viewed as having 
three other subcomplexes: \textbf{double check text book on this}
It is obvious that one of these base-level functional subcomplexes
will be in most limited supply, and that it will best represent the 
overall enzyme complex copy number.

Note that it is possible for an enzyme complex to catalyze different reactions. In fact, some transporter
complexes can transfer many different metabolites \textbf{across a lipid bilayer--up to} . Another example is the
