...
\vspace{10 mm} 
To my wife, Lin Xue, \\
For motivating me to pursue graduate school, \\
...
Dr. Kieran Smallbone, who has provided many algorithms that I have
employed, and one in particular that served as the impetus for part of
this work, Mr. Yiping Wang, a phenomenal young researcher who helped
...
friendship, stimulating scientific discussion, or some combination of
the three. These include Ms. Diana Chang, Dr. Hong Chian, Dr. Huifeng
Jiang, Ms. Haley Hunter-Zinck, Dr. Ben Heavner, Dr. Xiaojing Liu,
Mr. Lonnie Princehouse, and Mr. Kaixiong Ye.

Without the stimulus, confidence, and aid of my early mentors at the
...
undergraduate education, and to Dr. Hank Dietz, for being a great
...
in a cell. This isn't much of an issue for the simplest case where there is a reaction that is
...
Using some available tools, and modifying them where necessary, we attempt to formalize a model
for enzyme complex formation based on GPR (gene-protein-reaction) rules that are frequently available
in genome-scale annotations. For better curated models, the approach described immediately find
...
our English, we also include the notion of isozymes--that is, different proteins that catalyze the 
...
moment, we consider them to be part of the enzyme complex.

  Show how in F1, there are actually multiple copies of the same peptide. This is one of the examples where enzyme-complex stoichiometry is well known.
  In this particular case, we don't have as much to worry about since the peptide axle is not required in multiple copies for complex formation, and will likely be the minimum of the F1 subunit
\begin{figure}[H]
\caption{Illustration of the F1 part of the ATP Synthase complex.}
\label{fig:2F43}
\centering
...
The modeling of enzyme complex copy number can be thought of in a recursive fashion: each enzyme 
complex consists of several functional groups of proteins. These functional groups are required
for the enzyme complex to function (AND relationships). However, there may be alternative 
versions of each functional group (given by OR relationships). Eventually, the recursion terminates
with a base-level functional group  (i.e. a protein isoform).

Note that it is possible for an enzyme complex to catalyze different reactions. In fact, some transporter
complexes can transfer many different metabolites across a lipid bilayer--up to . Another example is the
ligation or hydrolysis of nucleotide, fatty acid, or peptide chains, where chains of different length may
all be substrates or products of the same enzyme complex. 

\emph{It may prove useful to formalize the above concept to show that what the algorithm below achieves is
in fact the copy number of this enzyme complex. I have attempted this in another document.}
...
\WHILE{$rule \neq AND(d_1,...,d_n)$ where $d_i$ has the form: $OR(g_1,...,g_m)$}
