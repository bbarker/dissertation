...
%Chapter 4:
%
% Add iteration loop to algorithm 2. 
...
Haley Hunter-Zinck, Ben Heavner, Xiaojing Liu, Anuttama Mohan,
...
\WHILE{$rxns_{irrev} > rxns_{irrev,prior}$}
  \IF {first iteration}
  \STATE {Constrain a user-specified flux (either forward or backward) $v_{scale}$ to a nonzero value.}
  \STATE {Scale $e_i$ so that $e_{scale} = v_{scale}$.}
  \ELSE 
